document.addEventListener("DOMContentLoaded", function () {
    ...
    // فلتر البرنامج بيتملى ببرامج قسم الأدمن الحالي بس (مش كل الأقسام)
    ...
    // كل قيمة فلتر (cs/swe/ai/is/cyber) بتتحط جنبها كلمات بديلة ممكن تتطابق
    // مع بيانات المشروع القديمة (زي "computer" أو "information") عشان الفلتر
    // يفضل شغال مع الداتا التجريبية اللي كانت موجودة قبل كده.
    const PROGRAM_MATCH_KEYWORDS = {
        cs: ["cs", "computer science", "computer"],
        swe: ["swe", "software engineering", "software"],
        ai: ["ai", "artificial intelligence"],
        is: ["is", "information systems", "information"],
    ...
    function filterProjects() {
        ...
        if (selectedProgram) {
            const keywords = PROGRAM_MATCH_KEYWORDS[selectedProgram] || [selectedProgram];
            list = list.filter(p => {
                if (!p.program) return true; // لو الخاصية مش موجودة في داتا المشروع ما يخفيش الصف
                const prog = p.program.toLowerCase();
                return keywords.some(keyword => prog.includes(keyword));
            });
        }
    ...
    function renderProjects(data) {
        ...
        data.forEach(project => {
            ...
            switch (project.status) {
                ...
                case "Pending Decision": badgeClass = "badge-decision"; break;
